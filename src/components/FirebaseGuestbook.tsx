          ...
          <div className="space-y-4">
            {entries.map((entry) => (
              <GuestbookEntry
                ...
                onSetReplyingTo={(entryId) => currentUser ? setReplyingTo(entryId) : signIn()}
                onReplyContentChange={setReplyContent}
                onSubmitReply={() => handleSubmitReply(entry.id)}
                onStartEditReply={(replyId: string) => currentUser && startEditReply({ id: replyId, content: '', createdAt: '', updatedAt: '', authorId: '', userId: '', author: { id: '', username: '' } })}
                onCancelEditReply={(replyId: string) => {
                  setEditingReply(null);
                  setEditContent('');
                }}
                onSaveEditReply={(replyId: string) => handleEditReply(replyId, editContent)}
